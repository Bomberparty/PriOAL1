...
int main()
{
    while(true)
    {
        ...
        string ans;
        long n;
        cout << "Для начала необходимо ввести число n." << endl << "Хозяин предпочитает консоль или файл для получения значения?[ф/к] ";
        getline(cin, ans);
        if (ans == "ф")
            n = file_input();
        else if (ans == "к")
        ...
        {
            cout << "Выберите одну из предложенных опций посредством введения соответствующей буквы!" << endl << endl;
            continue;
        }
        double farengate[n];
